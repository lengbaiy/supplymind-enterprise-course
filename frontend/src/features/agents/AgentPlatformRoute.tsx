...
import type { AgentApproval, AgentStep, AnalysisRun, MCPServer, UserMemory } from "../../app/domain-types";
import { AppShell } from "../../components/AppShell";
import { API_BASE, apiRequest } from "../../services/api";

type Tab = "runs" | "memory" | "mcp" | "approvals";
...
export function AgentPlatformRoute() {
  ...
  const load = useCallback(async () => {
    ...
    const [runsResult, memoriesResult, settingsResult, serversResult, approvalsResult] = await Promise.allSettled([
      ...
      api<AgentApproval[]>("/agent-approvals?status=pending"),
    ]);
  ...
  return <AppShell nav="Agent 平台" items={NAV_ITEMS} busy={loading} onNavigate={(item) => navigate(NAV_PATHS[item])} onRefresh={() => void load()} onLogout={() => { localStorage.clear(); navigate("/"); }}>
    <section className="agent-platform">
      <div className="agent-platform-heading"><div><span className="section-kicker">ENTERPRISE AGENT / CONTROL PLANE</span><h3>运行与治理</h3><p>查看分析路由、持久化记忆、受控工具连接和待审批操作。</p></div><a className="icon-button" href={`${API_BASE.replace("/api/v1", "")}/.well-known/agent-card.json`} target="_blank" rel="noreferrer" title="查看 A2A Agent Card" aria-label="查看 A2A Agent Card"><ExternalLink size={16} /></a></div>
