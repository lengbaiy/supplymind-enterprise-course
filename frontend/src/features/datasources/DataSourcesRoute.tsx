...
export function DataSourcesRoute() {
  ...
  const action = async (path: string, body?: unknown, method = "POST", successNotice = "") => { setBusy(true); setNotice(""); try { await api(path, { method, body: body === undefined ? undefined : JSON.stringify(body) }); await load(); if (successNotice) setNotice(successNotice); } catch (error) { setNotice(error instanceof Error ? error.message : "操作失败"); } finally { setBusy(false); } };
