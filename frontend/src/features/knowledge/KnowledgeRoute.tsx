...
export function KnowledgeRoute() {
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); setActiveAction("knowledge:create"); try { await api("/knowledge-bases", { method: "POST", body: JSON.stringify({ name: form.get("name"), description: form.get("description") || "" }) }); event.currentTarget.reset(); await load(); setNotice("知识库已创建"); } catch (error) { setNotice(error instanceof Error ? error.message : "创建失败"); } finally { setActiveAction(null); } };
  const upload = async (event: FormEvent<HTMLFormElement>, id: string) => { event.preventDefault(); const file = new FormData(event.currentTarget).get("file"); if (!(file instanceof File) || !file.name) { setNotice("请选择 PDF、Markdown 或 TXT 文件"); return; } setActiveAction(`knowledge:${id}:upload`); try { const body = new FormData(); body.append("file", file); const document = await api<Document>(`/knowledge-bases/${id}/documents`, { method: "POST", body }); setDocuments((current) => [document, ...current.filter((item) => item.id !== document.id)]); event.currentTarget.reset(); setNotice("文档已进入摄取队列，可在详情查看实时状态。"); } catch (error) { setNotice(error instanceof Error ? error.message : "文档上传失败"); } finally { setActiveAction(null); } };
